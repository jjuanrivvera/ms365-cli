...
// expansion. Built-ins always win, so an alias can never shadow `auth`, `mail`, etc.
...
func init() {
	metaRegistrars = append(metaRegistrars, func(d *deps) *cobra.Command {
		...
		setCmd := &cobra.Command{
			...
			Example: `  ms365 alias set inbox "mail list --folder inbox"
  ms365 alias set week "calendar events"`,
