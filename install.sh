#!/bin/sh
# ms365 installer for macOS and Linux.
#
...
#   MS365_VERSION=v1.2.3        pin a version (default: the latest release)
...
REPO="jjuanrivvera/ms365-cli"
BINARY="ms365"
VERSION="${MS365_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set MS365_VERSION"
fi
