...
// AES-256-GCM encrypted in a 0600 file. The encryption key comes from $MS365_KEYRING_PASSWORD
...
func deriveKey() ([]byte, error) {
	salt := []byte("ms365-credentials-v1")
	if pw := os.Getenv("MS365_KEYRING_PASSWORD"); pw != "" {
		return scrypt.Key([]byte(pw), salt, 1<<15, 8, 1, 32)
	}
	host, _ := os.Hostname()
	seed := fmt.Sprintf("ms365-fallback|%s|%d", host, os.Getuid())
...
func decrypt(b64 string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("decrypt failed (wrong $MS365_KEYRING_PASSWORD or host changed): %w", err)
	}
