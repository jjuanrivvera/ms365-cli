// Package config resolves ms365 configuration with a manual flag > env > file > default
// precedence (no Viper, per the cliwright house pattern). Profiles ("accounts") let one
// user hold several Microsoft 365 sign-ins side by side; tokens never live here — only in
// the OS keyring (see internal/auth).
package config
...
// Profile holds the non-secret settings for one Microsoft 365 account. The tokens live in
// the keyring under this profile's key; here is only what auth status displays and what a
// power user overrides (own app registration, sovereign cloud, display timezone).
type Profile struct {
	Username string `yaml:"username,omitempty" json:"username,omitempty"`   // UPN captured at auth login
	TenantID string `yaml:"tenant_id,omitempty" json:"tenant_id,omitempty"` // Entra tenant (realm)
	ClientID string `yaml:"client_id,omitempty" json:"client_id,omitempty"` // custom app registration override
	BaseURL  string `yaml:"base_url,omitempty" json:"base_url,omitempty"`   // sovereign-cloud Graph endpoint override
	Timezone string `yaml:"timezone,omitempty" json:"timezone,omitempty"`   // Prefer: outlook.timezone value
}
...
// Dir returns the configuration directory: $XDG_CONFIG_HOME/ms365 if set, else ~/.ms365-cli.
func Dir() (string, error) {
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "ms365"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".ms365-cli"), nil
}
...
// ResolveProfileName applies precedence: explicit flag > MS365_ACCOUNT > current_profile >
// default. (--account is the profile selector; --profile stays as a hidden alias.)
func (c *Config) ResolveProfileName(flag string) string {
	return FirstNonEmpty(flag, os.Getenv("MS365_ACCOUNT"), c.CurrentProfile, DefaultProfile)
}

// FirstNonEmpty returns the first non-empty string — the manual precedence helper used
// across ms365 instead of a config framework.
