...
func TestValidateBaseURL(t *testing.T) {
	assert.NoError(t, ValidateBaseURL("https://graph.microsoft.com/v1.0"))
...
func TestDirAndPath_XDG(t *testing.T) {
	...
	dir, err := Dir()
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(base, "ms365"), dir)
	p, err := Path()
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(base, "ms365", "config.yaml"), p)
}
...
func TestDir_HomeFallback(t *testing.T) {
	...
	dir, err := Dir()
	require.NoError(t, err)
	assert.Contains(t, dir, ".ms365-cli")
}
