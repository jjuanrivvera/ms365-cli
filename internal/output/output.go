// Package output renders Microsoft Graph JSON in table/json/yaml/csv/id. One renderer serves
...
// preferredOrder lists Graph fields that should lead a table when present, before the
// rest fall back to alphabetical. Deterministic ordering is required by GOAL.md §11.
var preferredOrder = []string{
	"id", "displayName", "userPrincipalName", "subject", "from.emailAddress.address",
	"mail", "receivedDateTime", "start.dateTime", "end.dateTime",
	"location.displayName", "organizer.emailAddress.address", "isRead",
	"hasAttachments", "bodyPreview", "webLink", "createdDateTime", "lastModifiedDateTime",
}
